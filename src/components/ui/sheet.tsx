  ...
  return (
    <SheetPrimitive.Backdrop
      ...
      className={mergeClassName(
        "fixed inset-0 z-50 bg-black/35 transition-opacity duration-150 data-ending-style:opacity-0 data-starting-style:opacity-0 motion-reduce:transition-none supports-backdrop-filter:backdrop-blur-xs",
        className,
      )}
      {...props}
    />
      ...
      >
        ...
        {showCloseButton ? (
          <SheetPrimitive.Close
            ...
            className={buttonClassName({ variant: "ghost", size: "icon", className: "absolute right-3 top-3" })}
...
function SheetTitle({ className, ...props }: SheetPrimitive.Title.Props) {
  return <SheetPrimitive.Title data-slot="sheet-title" className={mergeClassName("font-heading text-base font-medium text-foreground", className)} {...props} />;
}

function SheetDescription({ className, ...props }: SheetPrimitive.Description.Props) {
  return <SheetPrimitive.Description data-slot="sheet-description" className={mergeClassName("text-sm text-muted-foreground", className)} {...props} />;
}
